    ...
    >
      ...
      <div className="p-6 space-y-6">
        <div className="bg-tico-yellow/20 rounded-3xl p-6 text-center">
          ...
          <button className="bg-tico-black text-white font-bold py-3 px-6 rounded-2xl w-full flex items-center justify-center gap-2">
            <MessageCircle className="w-5 h-5" /> Iniciar Chat
          </button>
        </div>

        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          <SupportItem icon={PhoneCall} title="Llamar a emergencias" color="text-red-500" bg="bg-red-50" />
          <SupportItem icon={FileText} title="Reportar un problema con un viaje" />
          <SupportItem icon={FileText} title="Objetos perdidos" />
          <SupportItem icon={FileText} title="Preguntas frecuentes" />
        </div>
      </div>
    </motion.div>
  );
...
function SupportItem({ icon: Icon, title, color = "text-tico-black", bg = "bg-gray-100" }: { icon: any, title: string, color?: string, bg?: string }) {
  return (
